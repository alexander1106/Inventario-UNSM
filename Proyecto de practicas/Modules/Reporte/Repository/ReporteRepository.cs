        ...
        public async Task<ReporteResponseDto> ObtenerReporteAsync(ReporteRequestDto filtros)
        {
            ...
            switch (filtros.Tipo)
            {
                case 0: // INVENTARIO GENERAL
                    var invQuery = _context.Articulos // Verifica que en tu DbContext sea 'articulos' en minúscula
                    ...
                    {
                        invQuery = invQuery.Where(a =>
                            a.UbicacionId.HasValue &&
                            filtros.UbicacionIds.Contains(a.UbicacionId.Value));
                    }
                ...
                case 1: // PRÉSTAMOS
                    {
                        ...
                        // 🔥 FILTRO POR UBICACIÓN (por artículo)
                        if (filtros.UbicacionIds != null && filtros.UbicacionIds.Any())
                        {
                        ...
                        if (filtros.FechaFin.HasValue)
                            prestQuery = prestQuery.Where(p => p.FechaPrestamo <= filtros.FechaFin);

                        var prestamos = await prestQuery.AsNoTracking().ToListAsync();

                        response.Kpis.Add(new KpiDto
                        {
                            Label = "PRÉSTAMOS ACTIVOS",
                            Value = prestamos.Count(p => p.Estado == 1).ToString()
                        });

                        response.Kpis.Add(new KpiDto
                        {
                            Label = "ATRASADOS",
                            Value = prestamos.Count(p => p.FechaDevolucion < DateTime.Now && p.Estado == 1).ToString()
                        });
                ...
                case 2: // MANTENIMIENTO
                    {
                        ...
                        if (filtros.UbicacionIds != null && filtros.UbicacionIds.Any())
                        {
                            mantQuery = mantQuery.Where(m =>
                                m.Articulo != null &&
                                m.Articulo.UbicacionId.HasValue &&
                                filtros.UbicacionIds.Contains(m.Articulo.UbicacionId.Value)
                            );
                        }

                        if (filtros.FechaInicio.HasValue)
                            mantQuery = mantQuery.Where(m => m.FechaMantenimiento >= filtros.FechaInicio);

                        if (filtros.FechaFin.HasValue)
                            mantQuery = mantQuery.Where(m => m.FechaMantenimiento <= filtros.FechaFin);
                        ...
                        {
                            Label = "COSTO TOTAL",
                            Value = $"S/ {mantenimientos.Sum(m => m.Costo):N2}"
                        });

                        response.Kpis.Add(new KpiDto
                        {
                            Label = "EN MANTENIMIENTO",
                            Value = mantenimientos.Count(m => m.EstadoMantenimiento).ToString()
                        });
                ...
                case 3: // TRASLADOS
                    {
                        ...
                        if (filtros.FechaInicio.HasValue)
                            trasQuery = trasQuery.Where(t => t.FechaTraslado >= filtros.FechaInicio);
                        ...
                        response.Kpis.Add(new KpiDto
                        {
                            ...
                            Value = traslados.Count.ToString()
                        });
